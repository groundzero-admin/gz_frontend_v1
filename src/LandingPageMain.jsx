...
const FlipWords = ({ isDark }) => {
  const words = ["think", "build", "learn", "scale"];
...
const HeroHeading = ({ isDark }) => {
  const phrases = [
    ...
    { first: "learn faster", second: "ship faster" },
    { first: "dream bigger", second: "scale smarter" }
  ];
  const [index, setIndex] = useState(0);
...
const FormModal = ({ isOpen, onClose, url, title, isDark }) => {
  ...
  // Reset loading state whenever the modal opens
  useEffect(() => {
    if (isOpen) {
      setIsLoading(true);
    }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div 
        className={`relative w-full max-w-3xl h-[85vh] rounded-2xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200 ${isDark ? 'bg-[#13141F] border border-white/10' : 'bg-white border border-slate-200'}`}
      >
        {/* Header */}
        ...
        <div className={`flex-1 relative ${isDark ? 'bg-black' : 'bg-white'}`}>
           ...
           {/* LOADING STATE: Visible only when isLoading is true */}
           {isLoading && (
           ...
           {/* IFRAME: Hidden (opacity-0) until onLoad fires */}
           <iframe 
            src={url} 
            onLoad={() => setIsLoading(false)} // The key fix!
...
const LandingPageMain = () => {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'waitlist' | 'webinar' | null
  ...
  const styles = {
    ...
    card: isDark 
      ? "bg-[#13141F]/80 border-white/5 hover:border-cyan-500/40" 
      : "bg-white border-slate-200 shadow-xl shadow-slate-200/50 hover:border-cyan-500/40",
    cardHoverShadow: isDark 
      ? "hover:shadow-[0_10px_30px_-10px_rgba(34,211,238,0.2)]" 
      : "hover:shadow-2xl hover:shadow-cyan-500/10",
    iconBg: isDark ? "bg-slate-800/80" : "bg-slate-100",
    quoteBg: isDark ? "bg-[#13141F]/60 border-white/5" : "bg-white border-slate-200 shadow-xl",
    footer: isDark 
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center pt-6 px-4">
          <div className={`backdrop-blur-xl border rounded-[5px] px-6 py-3 flex items-center justify-between w-full max-w-5xl shadow-2xl transition-all duration-300 ${styles.navbar}`}>
            <div className="flex items-center gap-3">
              ...
              <span className={`font-bold text-lg tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>Ground Zero</span>
            </div>
        ...
        {/* ================= HERO SECTION ================= */}
        <section className="relative pt-44 pb-24 px-4 flex flex-col items-center justify-center text-center min-h-[85vh]">
          ...
          <p className={`mt-8 text-lg md:text-xl max-w-3xl mx-auto z-10 font-medium leading-relaxed transition-colors duration-300 ${styles.subtext}`}>
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-5 z-10">
            ...
            <button 
              ...
              className={`font-semibold px-8 py-3.5 rounded-lg transition-all hover:shadow-lg cursor-pointer backdrop-blur-md border ${isDark ? 'bg-[#13141F]/50 border-gray-600 hover:border-cyan-400 hover:text-cyan-400 text-white' : 'bg-white/50 border-slate-300 text-slate-700 hover:border-cyan-600 hover:text-cyan-600'}`}
            >
              Explore Programs
            </button>
          </div>
        ...
        {/* ================= WHAT WE DO ================= */}
        <section id="what-we-do" className="pt-32 pb-24 text-center relative z-10 scroll-mt-20">
        ...
        <section id="who-its-for" className="py-20 px-4 max-w-7xl mx-auto scroll-mt-20">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-16">People who find us most helpful</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { Icon: Rocket, title: "Founders", desc: "We help you cut through the chaos—clarify priorities, structure your to-dos, plan actionable next steps, and stay accountable." },
              { Icon: Compass, title: "Explorers & Builders", desc: "Narrow your direction, break down complex problems, get early answers fast, and learn to build effectively using AI tools." },
              { Icon: BookOpen, title: "Learners", desc: "Master AI-powered building while sharpening core thinking pillars: creativity, critical thinking, and systems thinking." }
            ].map((card, idx) => (
              <div key={idx} className={`backdrop-blur-lg border rounded-2xl p-8 text-center transition-all duration-300 group hover:-translate-y-1 ${styles.card} ${styles.cardHoverShadow}`}>
                ...
                <p className={`text-[15px] leading-relaxed ${styles.subtext}`}>{card.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="programs" className="py-20 px-4 max-w-7xl mx-auto scroll-mt-20">
          <div className="text-center mb-16">
            ...
            <p className={`text-lg max-w-2xl mx-auto ${styles.subtext}`}>Choose the format that fits your learning style, timeline, and goals.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
            {/* Solo Card */}
            <div className={`backdrop-blur-lg border rounded-2xl p-7 flex flex-col h-full transition-all duration-300 hover:-translate-y-1 ${styles.card} ${styles.cardHoverShadow}`}>
              ...
              <p className={`text-sm mb-6 leading-relaxed ${styles.subtext}`}>For individuals who want rapid, specific acceleration and private guidance.</p>
              ...
              <ul className="space-y-3 mb-8 flex-1">
                {["1:1 Live Sessions", "Unblocking specific hurdles", "Fully Self-paced", "Lock in sessions as you need them"].map((item, i) => (
                  <li key={i} className={`flex items-start gap-3 text-[15px] ${styles.subtext}`}>
            ...
            {/* Squad Card */}
            <div className={`backdrop-blur-lg border rounded-2xl p-7 flex flex-col h-full transition-all duration-300 hover:-translate-y-1 ${styles.card} ${isDark ? 'hover:border-blue-400/40 hover:shadow-[0_10px_30px_-10px_rgba(59,130,246,0.2)]' : 'hover:shadow-2xl hover:shadow-blue-500/10'}`}>
              ...
              <p className={`text-sm mb-6 leading-relaxed ${styles.subtext}`}>For peers who want to learn and build together using AI in a cohort.</p>
              <div className={`mb-6 p-4 rounded-xl border ${isDark ? 'bg-slate-800/50 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
              ...
              <div className="mt-auto">
                 {/* Waitlist Button - Triggers Modal */}
            ...
            {/* Sync Card */}
            <div className={`backdrop-blur-lg border rounded-2xl p-7 flex flex-col h-full transition-all duration-300 hover:-translate-y-1 ${styles.card} ${styles.cardHoverShadow} relative overflow-hidden`}>
              ...
              <p className={`text-xs font-bold mb-4 uppercase tracking-wider ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>Discovery</p>
              ...
              <div className="flex-1 z-10">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                     {/* Webinar Button - Triggers Modal */}
                     <button onClick={openWebinar} className={`text-sm font-bold hover:underline flex items-center gap-1 ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>
                       Join our next webinar <span aria-hidden="true">&rarr;</span>
                     </button>
                     <span className={`border text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${isDark ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-300' : 'bg-cyan-100 border-cyan-200 text-cyan-700'}`}>Free</span>
                  </div>
              </div>
        ...
        <section id="about" className="py-20 px-4 max-w-6xl mx-auto scroll-mt-20">
          ...
          <div className="grid grid-cols-1 gap-8">
            {/* Quote Card */}
            <div className={`backdrop-blur-md rounded-2xl p-10 md:p-12 relative overflow-hidden transition-all duration-300 ${styles.quoteBg}`}>
              ...
              <p className={`text-xl md:text-2xl leading-relaxed relative z-10 font-light font-serif italic ${isDark ? 'text-gray-200' : 'text-slate-700'}`}>
                Whether you want to build a company, accelerate your career or simply navigate a noisy world with clarity, <span className={`font-semibold ${isDark ? 'text-white' : 'text-slate-900'}`}>you need a system.</span>
                <br /><br />
            ...
            {/* Founder Profile */}
            <div className={`backdrop-blur-md border rounded-2xl p-8 flex flex-col md:flex-row items-center md:items-start gap-8 shadow-lg transition-all ${isDark ? 'bg-[#13141F]/80 border-white/5 hover:border-white/10' : 'bg-white border-slate-200 hover:border-cyan-200'}`}>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="flex gap-4 mt-5 justify-center md:justify-start">
                  <a href="#" className={`p-2.5 rounded-xl transition-all shadow-md group ${isDark ? 'bg-slate-800/50 hover:bg-cyan-500/20 hover:text-cyan-400 hover:shadow-cyan-500/10' : 'bg-slate-100 hover:bg-cyan-100 hover:text-cyan-600'}`}><Twitter size={18} className={`${isDark ? 'text-gray-400' : 'text-slate-400'} group-hover:text-current transition-colors`} /></a>
                  <a href="#" className={`p-2.5 rounded-xl transition-all shadow-md group ${isDark ? 'bg-slate-800/50 hover:bg-blue-500/20 hover:text-blue-400 hover:shadow-blue-500/10' : 'bg-slate-100 hover:bg-blue-100 hover:text-blue-600'}`}><Linkedin size={18} className={`${isDark ? 'text-gray-400' : 'text-slate-400'} group-hover:text-current transition-colors`} /></a>
                </div>
              </div>
            </div>
          </div>
        ...
        <footer className={`py-24 px-4 text-center border-t backdrop-blur-lg relative z-10 transition-colors duration-300 ${styles.footer}`}>
          <h2 className={`text-2xl md:text-3xl font-bold mb-8 max-w-3xl mx-auto leading-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>
