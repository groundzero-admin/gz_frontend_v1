    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h2 className="font-bold text-2xl">GZ Teacher</h2>
        </div>

        {/* User email (new line) */}
        {userData?.email && (
          <span
            className="text-sm font-medium ml-11 opacity-70"
            style={{
      ...
      <ul className="flex flex-col gap-2 flex-grow">
        <SidebarLink
          ...
          label="Dashboard"
          isActive={isExactActive("/teacher/dashboard")}
          isDark={isDark}
        />
        <SidebarLink
        ...
        <SidebarLink
          ...
          label="Doubts"
          isActive={isActive("/teacher/dashboard/doubts")}
          isDark={isDark}
        />
        <SidebarLink
          ...
          icon={<FaCalendarAlt />}
          label="Schedules"
          isActive={isActive("/teacher/dashboard/schedules")}
          isDark={isDark}
        />
      </ul>
...
const TeacherLayout = () => {
  ...
  useEffect(() => {
    const authorizePage = async () => {
      const response = await checkRole("teacher")
      if (response.success) {
        setAuthStatus({
          isLoading: false,
          isAuthorized: true,
  ...
  // --- RENDER 2: Unauthorized ---
  if (!authStatus.isAuthorized) {
    const isRoleMismatch =
      authStatus.correctRole && authStatus.correctRole !== "teacher"
    return (
      <FullPageMessage isDark={isDark}>
