...
import { validateTeacherInvite, completeTeacherOnboarding } from "../api.js";

// --- Reusable Input Component with Auto-fill Fix ---
const SignupInput = ({ id, label, icon, isDark, error, ...props }) => (
  <div className="mb-4">
    ...
    <div className="relative group">
      ...
      <input
        id={id}
        autoComplete="off" // Helps prevent some unwanted autofill menus
        ...
        style={{
          ...
          // CSS Hack to override browser Auto-fill background color
...
const TeacherSignupPage = () => {
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      const response = await validateTeacherInvite(token);
  ...
  const handleChange = (e) => {
    ...
    if (id === 'mobile') {
        if (!/^\d*$/.test(value)) return; // If not a digit, ignore input
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await completeTeacherOnboarding(payload);
  ...
  // --- Styles ---
  // Fancy mesh gradient background
  const bgStyle = {
    background: isDark 
    ...
    <div style={bgStyle}>
      ...
      {step === 'VALIDATING' && (
        <div className="animate-pulse flex flex-col items-center">
          <FaChalkboardTeacher className="text-6xl text-[var(--accent-teal)] mb-6" />
      ...
      {step === 'SUCCESS' && (
        <div className="max-w-md w-full p-8 rounded-2xl border text-center" style={cardStyle}>
          ...
          <p className="opacity-70 mb-8">
            Welcome to the team. Your teacher account is ready.
          </p>
          <button 
             onClick={() => navigate('/login')}
        ...
        <div className="max-w-2xl w-full p-8 rounded-3xl border relative overflow-hidden" style={cardStyle}>
          ...
          <div className="text-center mb-8">
            <div className="inline-block p-4 rounded-full bg-[var(--accent-teal)] bg-opacity-10 mb-4">
              <FaChalkboardTeacher className="text-4xl text-[var(--accent-teal)]" />
            </div>
            {/* Added "Teacher Registration" label as requested */}
            <p className="text-sm font-bold tracking-widest text-[var(--accent-teal)] uppercase mb-2">Teacher Registration</p>
            <h1 className="text-3xl font-bold">Complete Your Profile</h1>
            <p className="opacity-60 mt-2 text-sm">Fill in the details below to activate your account</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-6">
            ...
            <div>
              ...
              <SignupInput
                ...
                maxLength={10} 
                type="text" // Using text type but regex controlled for integers
                inputMode="numeric"
              />
            </div>
